...
pub(crate) struct ElementEndTag {
    lt: syn::token::Lt,
    pub(crate) name: proc_macro2::Ident,
    gt: syn::token::Gt,
}

impl Parse for ElementEndTag {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let lt = input.parse()?;
        input.parse::<syn::token::Slash>()?;
        let name = input.parse()?;
        let gt = input.parse()?;
        Ok(ElementEndTag { lt, name, gt })
    }
}
...
impl ElementEndTag {
    ...
    pub(crate) fn peek(start_tag_name: &proc_macro2::Ident, input: ParseStream) -> bool {
